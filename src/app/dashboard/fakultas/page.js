...
const { data: stepsData, error: stepsError } = await supabase
  ...
  .order("updated_at", { ascending: true });
        ...
        const finalSteps = (stepsData || []).map((step) => {
          ...
          if (step.role === "ADMIN") {
            fallbackCatatan = suratData?.catatan_admin;
          }

          if (step.role === "SEKRETARIS") {
            fallbackCatatan = suratData?.catatan_sekretaris;
          }

          if (
            step.role === "WAREK" ||
            step.role === "WAKIL_REKTOR"
          ) {
            fallbackCatatan = suratData?.catatan_wakil_rektor;
          }

          if (step.role === "REKTOR") {
            fallbackCatatan = suratData?.catatan_rektor;
          }
...
const lastApproved = [...(steps || [])]
  ...
  // PENOLAK & ALASAN
  // =========================
  const rejectedBy = surat?.catatan_rektor
    ? "Rektor"
              ...
              {surat?.status === "rejected" && (
                ...
                >
                  ...
                  <div>
                    ...
                    >
                      &ldquo;
                      {rejectedReason || "Tidak ada catatan"}
                      &rdquo;
                    </div>
                  </div>
                </div>
...
export default function DashboardFakultas() {
  ...
  useEffect(() => {
    (async () => {
      ...
      if (approvedIds.length > 0) {
        ...
        const map = {};
        (stepData || []).forEach((s) => { if (!map[s.surat_id]) map[s.surat_id] = s; });
  ...
  return (
    <div style={{ flex: 1, overflowY: "auto", padding: "22px 24px", background: "#f4f6f9", minHeight: "100vh" }}>
      <div style={{ fontSize: 10, color: "#888", textTransform: "uppercase", letterSpacing: "0.08em", marginBottom: 4 }}>Academic Overview</div>
      <div style={{ fontSize: 22, fontWeight: 600, color: "#111", marginBottom: 20 }}>Dashboard Overview</div>
      ...
      <div style={{ background: "#fff", border: "0.5px solid #e5e7eb", borderRadius: 10, overflow: "hidden" }}>
        ...
        {loading ? (
          <div style={{ padding: 40, textAlign: "center", color: "#aaa", fontSize: 13 }}>Memuat data...</div>
        ) : suratList.length === 0 ? (
          <div style={{ padding: 40, textAlign: "center", color: "#aaa", fontSize: 13 }}>Belum ada pengajuan surat</div>
        ) : (
          <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 12 }}>
            <thead>
              <tr style={{ background: "#f9fafb" }}>
                {["No. Surat", "Jenis Surat", "Tanggal", "Status", "Aksi"].map((h) => (
                  <th key={h} style={{ padding: "10px 18px", textAlign: "left", fontSize: 10, fontWeight: 600, color: "#888", borderBottom: "0.5px solid #e5e7eb", textTransform: "uppercase", letterSpacing: "0.06em" }}>{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {suratList.map((row, i) => {
                ...
                return (
                  <tr key={row.id} style={{ borderBottom: i < suratList.length - 1 ? "0.5px solid #e5e7eb" : "none" }}>
                    ...
                    <td style={{ padding: "11px 18px", fontWeight: 500 }}>{row.templates?.nama_template || "—"}</td>
                    ...
                    <td style={{ padding: "11px 18px" }}>
                      <div onClick={() => setSelectedSuratId(row.id)} title="Lihat detail & alur persetujuan" style={{ width: 26, height: 26, borderRadius: 6, border: "0.5px solid #e5e7eb", background: "#fff", display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer" }}>
