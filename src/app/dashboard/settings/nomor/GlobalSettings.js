  ...
  return (
    <div style={{ background: "#fff", border: "0.5px solid rgba(0,0,0,0.1)", borderRadius: 12, overflow: "hidden", marginBottom: 12 }}>
      <div style={{ display: "flex", alignItems: "center", gap: 8, padding: "0.85rem 1.1rem", borderBottom: "0.5px solid rgba(0,0,0,0.08)" }}>
        ...
        <span style={{ fontSize: 13, fontWeight: 500, color: COLORS.navy }}>{title}</span>
      </div>
    ...
    {currentNumber !== "..." && (
      <span style={{ fontSize: 11, color: "rgba(255,255,255,0.3)", fontFamily: "monospace" }}>
        ...
        <div style={{ marginBottom: 10 }}>
          ...
          <div style={{ display: "flex", flexWrap: "wrap", alignItems: "center", gap: 4, minHeight: 44, padding: "8px 10px", background: "#f8fafc", border: "0.5px solid rgba(0,0,0,0.1)", borderRadius: 8, marginTop: 5 }}>
            ...
            {activeTokens.map((t, i) => (
              <span key={t + i} style={{ display: "flex", alignItems: "center", gap: 4 }}>
                <button
                  ...
                  style={{ display: "flex", alignItems: "center", gap: 5, fontFamily: "monospace", fontSize: 11, fontWeight: 500, padding: "4px 9px", borderRadius: 6, border: "0.5px solid rgba(0,0,0,0.12)", background: "#fff", color: "#334155", cursor: "pointer" }}
              ...
              {unusedTokens.map((t) => {
                ...
                return (
                  <button
                    ...
                    onMouseEnter={(e) => { e.currentTarget.style.background = isCustom ? "#f5f3ff" : "#f1f5f9"; e.currentTarget.style.borderStyle = "solid"; }}
        ...
        {/* Format string */}
        <div style={{ padding: "9px 12px", background: "#f8fafc", border: "0.5px solid rgba(0,0,0,0.08)", borderRadius: 8, fontFamily: "monospace", fontSize: 12, color: "#475569", marginBottom: 4 }}>
          {currentFormat}
        </div>
        <div style={hintStyle}>Format ini disimpan ke database dan dipakai untuk generate nomor otomatis.</div>
      </Card>
          ...
          <thead>
            <tr style={{ background: "#f9fafb" }}>
              {["Token", "Keterangan", "Contoh"].map((h) => (
                <th key={h} style={{ padding: "8px 10px", textAlign: "left", fontSize: 10, fontWeight: 600, color: "#888", borderBottom: "0.5px solid #e5e7eb", textTransform: "uppercase" }}>
                  {h}
                </th>
              ))}
            </tr>
          ...
          <tbody>
            {SYSTEM_TOKENS.map((t, i, arr) => (
              <tr key={t} style={{ borderBottom: i < arr.length - 1 ? "0.5px solid #f3f4f6" : "none" }}>
                <td style={{ padding: "8px 10px", fontFamily: "monospace", fontSize: 11, fontWeight: 600, color: "#374151" }}>{t}</td>
                <td style={{ padding: "8px 10px", color: "#555" }}>{SYSTEM_TOKEN_DESC[t]}</td>
