...
export default function TemplateEditor({ template }) {
  ...
  const handleGenerate = async () => {
    try {
      ...
      a.href = url; a.download = "hasil.docx"; a.click();
    } catch {
  ...
  if (!tujuan) { alert("Pilih tujuan terlebih dahulu"); return; }
  setSubmitting(true);
  try {
      ...
      <div className="bg-white rounded-2xl shadow h-full flex flex-col">
        ...
        <div className="flex flex-col lg:flex-row flex-1 overflow-hidden">
          <div className="w-full lg:w-80 shrink-0 border-b lg:border-b-0 lg:border-r p-6 space-y-4 overflow-auto">
            <p className="text-xs text-gray-400 uppercase tracking-wide font-semibold">Variabel</p>
            {fields.length === 0 ? (
              <p className="text-gray-400 text-sm">Tidak ada variabel</p>
            ) : (
              fields.map(field => (
                <div key={field}>
                  <label className="block text-sm mb-1 font-medium text-gray-700">{field}</label>
                  <input
                    type="text"
                    value={values[field] || ""}
          ...
          <div className="flex-1 overflow-auto p-8 bg-gray-50">
            <div className="max-w-2xl mx-auto bg-white shadow-sm rounded-xl p-8 min-h-full">
              <p className="text-xs text-gray-400 uppercase tracking-wide font-semibold mb-4">Preview Dokumen</p>
              <div
