  ...
  return (
    <div className="bg-white rounded-2xl shadow h-full flex flex-col">
      <div className="p-6 pb-4">
        <input type="text" placeholder="Cari template..." className="w-full bg-gray-100 border border-gray-300 rounded-xl py-3 px-4 text-sm focus:outline-none focus:border-blue-500 transition-all" />
      </div>
      ...
      <div className="flex-1 overflow-auto px-3 pb-6">
        {templates.length === 0 ? (
          ...
          templates.map((template) => (
            <div key={template.id} className="mx-3 mb-3 p-4 rounded-2xl border border-transparent hover:border-gray-200 hover:bg-gray-50 transition-all group">
              <div className="flex justify-between items-start">
                <div onClick={() => onSelect && onSelect(template)} className="cursor-pointer flex-1">
                  <div className="flex items-center gap-2">
                    <div className="font-semibold text-gray-800 text-[15px]">{stripExt(template.nama_template)}</div>
                ...
                <div className="flex gap-1 opacity-100 sm:opacity-0 sm:group-hover:opacity-100 transition-opacity">
                  <button onClick={() => handleEdit(template)} className="p-2.5 hover:bg-blue-50 rounded-xl text-blue-600 transition"><Pencil size={18} /></button>
                  <button onClick={() => handleDelete(template.id)} className="p-2.5 hover:bg-red-50 rounded-xl text-red-500 transition"><Trash2 size={18} /></button>
                </div>
              </div>
            </div>
          ))
        ...
        <div className="fixed inset-0 bg-black/50 flex items-center justify-center z-100 backdrop-blur-sm p-4">
          <div className="bg-white rounded-3xl shadow-2xl w-full max-w-md overflow-hidden">
            <div className="px-6 pt-6 pb-4 flex items-center justify-between">
              ...
              <button onClick={() => setEditData(null)} className="p-2 hover:bg-gray-100 rounded-xl transition text-gray-400"><X size={20} /></button>
            </div>
            ...
            <div className="px-6 pb-6 space-y-5">
              ...
              <div>
                ...
                <input type="text" value={editData.nama_template}
              ...
              <div>
                ...
                <div className="grid grid-cols-2 gap-2">
                  {JENIS_OPTIONS.map((opt) => {
                    ...
                    return (
                      <button key={opt.value} onClick={() => setEditData({ ...editData, jenis_surat: opt.value })}
                        className={`flex flex-col items-start p-3 rounded-2xl border-2 transition-all text-left ${active ? "bg-indigo-50 border-indigo-400" : "bg-gray-50 border-transparent hover:border-gray-200"}`}>
                        <span className={`text-base font-bold ${active ? "text-indigo-700" : "text-gray-500"}`}>{opt.value}</span>
                        <span className={`text-[11px] mt-0.5 ${active ? "text-indigo-500" : "text-gray-400"}`}>{opt.label}</span>
                      </button>
                    );
                  })}
                </div>
                  ...
                  {ROLES.map((role) => {
                    ...
                    return (
                      <button key={role} onClick={() => toggleRole(role)}
                        className={`flex items-center gap-3 p-3 rounded-2xl border-2 transition-all text-left ${active ? `${c.bg} ${c.border}` : "bg-gray-50 border-transparent hover:border-gray-200"}`}>
                        <div className={`w-8 h-8 rounded-xl flex items-center justify-center shrink-0 ${active ? c.check + " shadow-sm" : "bg-gray-200"}`}>
                          {active ? <Check size={15} className="text-white" strokeWidth={3} /> : <div className="w-2 h-2 rounded-full bg-gray-400" />}
                        </div>
                        <span className={`text-sm font-semibold ${active ? c.text : "text-gray-500"}`}>{role}</span>
                      </button>
                    );
                  })}
                </div>
              ...
              <div className="flex gap-3 pt-1">
                <button onClick={() => setEditData(null)} className="flex-1 py-3 font-medium text-gray-600 hover:bg-gray-100 rounded-2xl transition text-sm">Batal</button>
                <button onClick={handleSaveEdit} className="flex-1 py-3 bg-blue-600 hover:bg-blue-700 text-white font-semibold rounded-2xl shadow transition text-sm">Simpan</button>
              </div>
            </div>
          </div>
        </div>
