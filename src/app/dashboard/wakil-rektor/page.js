...
function Modal({ surat, onClose, onAction }) {
  const [catatan, setCatatan] = useState('');  // baris 27
  const [loading, setLoading] = useState(false); // baris 28
                                                 // baris 29 kosong
  if (!surat) return null;                       // baris 30 ← harus di sini
                                                 // baris 31 kosong
  const isLastStep = surat.tujuan === 'WAREK';  // baris 32
  const approveLabel = isLastStep ? 'Setujui' : 'Teruskan ke Rektor'; // baris 33
  const ApproveIcon = isLastStep ? CheckCircle : ChevronRight; // baris 34
    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{
          ...
          <div>
            <p style={{ margin: 0, fontSize: 12, color: '#6b7280', fontWeight: 600, textTransform: 'uppercase', letterSpacing: 0.5 }}>Detail Surat</p>
        ...
        <div style={{ padding: '1.25rem 1.5rem', borderBottom: '1px solid #f3f4f6' }}>
          ...
          {surat.catatan_sekretaris && (
            <div style={{ marginTop: 12, background: '#fdf6e7', border: '1px solid #c9993a33', borderRadius: 8, padding: '10px 14px' }}>
              <p style={{ margin: '0 0 4px', fontSize: 11, color: '#c9993a', fontWeight: 600, textTransform: 'uppercase', letterSpacing: 0.4 }}>Catatan Sekretaris Rektor</p>
...
export default function DashboardWakilRektor() {
  ...
  // ✅ fetchSurat dipindah ke dalam useEffect
  useEffect(() => {
  ...
  // ✅ handleAction pakai setSuratList langsung, tidak perlu fetchSurat()
    ...
    <div style={{ minHeight: '100vh', background: '#f1f5f9', fontFamily: 'system-ui, sans-serif' }}>
      <div style={{ background: '#1a2744', padding: '1.5rem 2rem' }}>
        <div style={{ maxWidth: 1100, margin: '0 auto' }}>
          ...
          <h1 style={{ margin: '6px 0 0', color: '#fff', fontSize: 22, fontWeight: 700 }}>Dashboard Wakil Rektor</h1>
        </div>
      </div>
